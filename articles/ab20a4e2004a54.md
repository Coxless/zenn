---
title: "WSL + Arch Linux + nixで環境構築してみた"
emoji: "🦔"
type: "tech" # tech: 技術記事 / idea: アイデア
topics: [wsl, nix]
published: false
---

## はじめに

WSL のディストリビューションはこれまで Ubuntu や Debian を使っていましたが、ドライブを圧迫してきたためクリーンインストールしようと思いました。
どうせクリーンインストールするなら、最小構成でスッキリさせたいと思い、Arch Linux を選びました。

ただ Arch Linux は素の状態だとパッケージ管理や dotfiles の管理を自前でやる必要があり、環境を再現するのが大変です。
そこで **nix + home-manager** を使って、宣言的に環境を管理する構成にしてみました。

---

## 前提環境

| 項目 | 内容 |
|------|------|
| OS | Windows 11 |
| WSL | WSL 2 |
| ディストリビューション | Arch Linux |
| nix インストーラ | Determinate Nix Installer |
| 環境管理 | home-manager |

---

## 手順

### 1. Arch Linux のインストール

WSL に Arch Linux をインストールします。

```bash
wsl --install archlinux --name main
```

:::message
`--name` オプションで任意の名前を付けられます。つけなくてもいいです。
:::

---

### 2. 初期設定

インストール直後は root ユーザーでログインした状態になります。以下の順で初期設定を進めます。

**パッケージの更新とインストール**

pacmanのミラーリストを更新し、nix をインストールするために必要なパッケージを入れておきます。

```bash
pacman -Syyu
pacman -S sudo vim curl git
```

**ユーザーの作成**

ユーザ名は `wslUser` としています。必要に応じて変更してください。

```bash
useradd -m -G wheel -s /bin/bash wslUser
passwd wslUser
```

**sudo の設定**

```bash
EDITOR=vim visudo
```

`visudo` を開いたら、以下の行のコメントアウトを外します。

```
%wheel ALL=(ALL) ALL
```

**デフォルトユーザーの設定**

`/etc/wsl.conf` を作成・編集し、WSL 起動時のデフォルトユーザーを指定します。

```ini
[user]
default = wslUser
```

設定後、WSL を再起動して反映させます。

```bash
wsl --shutdown
wsl -d main
```

---

### 3. nix のインストール

[Determinate Systems の nix-installer](https://github.com/DeterminateSystems/nix-installer) を使います。公式インストーラよりも WSL との相性が良く、アンインストールも簡単です。

また、flakeが最初から有効になっているのも便利です。

```bash
curl -fsSL https://install.determinate.systems/nix | sh -s -- install
```

インストール後、シェルを再起動して nix コマンドが使えることを確認します。

```bash
nix --version
```

---

### 4. home-manager で環境設定

home-manager を使って、パッケージや dotfiles を宣言的に管理します。

**設定ファイルの準備**

`~/.config/home-manager/` 以下を設定します。
- flake.nix: home-manager のフレーク定義ファイル
- home.nix: home-manager の設定ファイル

自分はdotfilesのように、home-manager用のリポジトリを作成しクローンするようにしました。

参考:
https://github.com/Coxless/home-magager

一から設定する場合は、以下でテンプレートを生成できるのでそれを編集してください。
nix run home-manager/master -- init --switch

そのほかはhome-manager のドキュメントを参考にしてください。

公式マニュアル:
https://nix-community.github.io/home-manager/

参考になる記事:
https://blog.tomoyukim.net/entry/nix-flake-home-manager/

**適用**

```bash
nix run home-manager/master -- switch
home-manager switch　// 二回目以降
```

[!home-manager switch 成功](images\ab20a4e2004a54\home-manager-switch.png)

---

## まとめと感想

- **Arch Linux** は Ubuntu より軽量で、不要なパッケージが少ない分スッキリしています
- **nix + home-manager** を使うことで、`home.nix` を Git 管理するだけで環境を再現できるのが便利です
- 初期設定の手間はありますが、一度構築してしまえばあとは `home-manager switch` だけで済むのは楽でした

---

## おわりに

WSL で Arch Linux + nix を使っている方、他に便利な設定があればぜひコメントで教えてください。
home-manager の設定例やフィードバックも歓迎です！
